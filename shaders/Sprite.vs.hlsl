...
// 여기서는 픽셀 좌표를 받아 NDC(-1~1)로만 바꿔주고, 아틀라스 UV와 색은 그대로 넘긴다.
// 스프라이트의 실제 위치·크기는 이미 C++이 정점에 구워 넣었으므로 좌표계 변환만 담당한다.
...
// 뒤에 붙은 POSITION, TEXCOORD, COLOR가 시맨틱 이름이다. C++ 입력 레이아웃과 글자까지 같아야 한다.
struct VSInput
{
    float2 position : POSITION;   // 화면 픽셀 좌표 (좌상단 원점, y 아래로 +)
    float2 uv       : TEXCOORD;   // 아틀라스 UV (0~1)
    float4 color    : COLOR;
};
...
VSOutput main(VSInput input)
{
    ...
    output.uv       = input.uv;      // 보간되어 픽셀 셰이더로 간다
